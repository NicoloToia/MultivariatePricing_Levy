...
% fun = @(omega) (exp(zeroRate_US*ttm - 0.5*sigma_US^2*rho^2.*ttm + sigma_US*rho*omega).*...
%         cdf('Normal',((zeroRate_US-0.5*sigma_US.^2)*ttm + sigma_US*(ttm*(1-rho^2)*sigma_US + rho*omega))/(sigma_US*sqrt(ttm*(1-rho^2))), 0, sqrt(ttm)) ...
%         - cdf('Normal', ((zeroRate_US-0.5*sigma_US^2)*ttm + rho*omega*sigma_US)/(sigma_US*sqrt(ttm*(1-rho^2))), 0, sqrt(ttm)))...
%         .* 1/sqrt(2*pi*ttm) .* exp(-0.5 * omega.^2./ttm);
