function plot3d_impl_vol_new(Filtered_Market)
% 3D plot
...
for ii = 1:length(Filtered_Market.datesExpiry)
    ...
    % Espandi le griglie con i nuovi dati
...
% size(xGrid_new)
% size(yGrid_new)
zGrid_new_interplated = zGrid_new;
for ii = 8 : length(x_expiries)
...
zGrid_new_interpolated;
zGrid_new_plot = [zGrid_new(:,1:8), zGrid_new_interpolated(:,9:end)];
surf(xGrid_new, yGrid_new, zGrid_new_plot);
xlabel('Expiries');
ylabel('Strikes');
zlabel('Implied Volatilities');
