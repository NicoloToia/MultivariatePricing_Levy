...
x_yearly = linspace(1,length(Returns.Daily(:,1)),length(Returns.Annually(:,1)));
% Daily returns linspace
x_daily = 1:length(Returns.Daily(:,1));

% Plot the returns
figure;
% Plot the yearly returns EU and US
plot(x_yearly,flip(EU_Market.spot.*ret2price(Returns.Returns.Annually(:,2))),  '--', 'Color', [0.3 0.75 0.93], 'LineWidth', 2);
hold on;
plot(x_yearly,flip(US_Market.spot.*ret2price(Returns.Returns.Annually(:,1))), '--', 'Color', [1 0.6 0.78], 'LineWidth', 2);
% Plot the daily returns EU and US
plot(x_daily,flip(EU_Market.spot.*ret2price(Returns.Returns.Daily(:,2))), 'b', 'LineWidth', 2);
plot(x_daily,flip(US_Market.spot.*ret2price(Returns.Returns.Daily(:,1))), 'r', 'LineWidth', 2); 
